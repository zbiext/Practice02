...
public abstract class BaseTurboAdapter<T, VH extends BaseViewHolder> extends AbsTurboAdapter<T, BaseViewHolder> implements OnLoadMoreListener {
    ...
    public final int getItemViewType(int position) {
        if (mHeaderView != null && position == 0) {
            Log.d(TAG, "getItemViewType: TYPE_HEADER_VIEW");
            return TYPE_HEADER_VIEW;
        } else if (mEmptyView != null && getItemCount() == 1 && mEmptyEnable) {
            Log.d(TAG, "getItemViewType: TYPE_EMPTY_VIEW");
            return TYPE_EMPTY_VIEW;
        } else if (position == mData.size() + getHeaderViewCount()) {
            if (mLoading) {
                Log.d(TAG, "getItemViewType: TYPE_LOADING_VIEW");
                return TYPE_LOADING_VIEW;
            } else if (mFooterView != null) {
                Log.d(TAG, "getItemViewType: TYPE_FOOTER_VIEW");
                return TYPE_FOOTER_VIEW;
            }
        }
        int defItemViewType = getDefItemViewType(position);
        Log.d(TAG, "getItemViewType: getDefItemViewType(position) = " + defItemViewType);
